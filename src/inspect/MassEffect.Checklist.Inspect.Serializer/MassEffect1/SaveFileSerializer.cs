...
public class SaveFileSerializer : ISaveFileSerializer<MassEffect1SaveFile>
{
    public async Task<MassEffect1SaveFile> DeserializeAsync(Stream stream, CancellationToken token = default)
    {
        ...
        if (compressedReader.ReadUInt32() != 0x9E2A83C1) throw new InvalidOperationException("Invalid save file magic number");
        ...
        foreach (var chunkBytes in chunkedHeaders.Select(chunk => compressedReader.ReadBytes((int)chunk.CompressedSize)))
        {
